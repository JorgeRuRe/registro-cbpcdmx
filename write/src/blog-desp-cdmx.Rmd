...
pacman::p_load(tidyverse, sf, here, readxl, janitor, png, cowplot)
```
...
files <- list(registro_cbpcdmx_clean = here("import", "output", "registro_cdmx_clean.rds"),
              ...
              perfiles_estatus_cnb = here("descriptives-rnpdno", "output", "perfiles_estatus_cnb.png"))
```
...
imagen_perfiles_cnb <- readPNG(files$perfiles_desp_cnb)
imagen_perfiles_cdmx <- readPNG(files$perfiles_desp)

p1 <- ggdraw() + draw_image(imagen_perfiles_cnb) 
p2 <- ggdraw() + draw_image(imagen_perfiles_cdmx)


p1 + p2 +
      theme_minimal(base_family = "Courier New") +
      plot_annotation(
   title = "Perfiles de personas desaparecidas a nivel nacional \n en comparación con la CDMX"
   ) &
...
imagen_perfiles_estatus_cnb <- readPNG(files$perfiles_estatus_cnb)
imagen_perfiles_estatus_cdmx <- readPNG(files$perfiles_desp_estatus)

p3 <- ggdraw() + draw_image(imagen_perfiles_estatus_cnb) 
p4 <- ggdraw() + draw_image(imagen_perfiles_estatus_cdmx)


p3 + p4 +
      theme_minimal(base_family = "Courier New") +
      plot_annotation(
   title = "Perfiles de personas desaparecidas a nivel nacional \n en comparación con la CDMX con base en estatus de localización"
   ) &
   theme(plot.title = element_text(face = "bold", hjust = 0.5))
```
